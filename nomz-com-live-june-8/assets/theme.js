...
(function($){
    ...
    QtySelector.prototype = $.extend({}, QtySelector.prototype, {
      initEventListeners: function() {
        ...
        this.$add.on('click', function() {
          this.add(this.$add.data('variant-id'));
        }.bind(this));
      },
      ...
      add: function(variant) {
        $('body').trigger('qty' + this.options.namespace + '--add', variant);
      },
  ...
  theme.CartDrawer = (function() {
    ...
    CartDrawer.prototype = $.extend({}, CartDrawer.prototype, {
      ...
      buildCart: function(cart, openDrawer) {
        ...
        let upsellItemVariants = []
        $('.js-qty__atc').each(function(index, btn){
          upsellItemVariants.push($(btn).data('variant-id'))
        })
        ...
        if (cart.item_count === 0) {
          ...
          var upsellItems = this._upsellOptions = document.querySelector('[data-upsell-options]') ? JSON.parse(document.querySelector('[data-upsell-options]').innerHTML) : []

          $.each(cart.items, function(index, product) {
            
            for (let variant of upsellItemVariants) {
              if (product.variant_id === variant){
                for (var i = 0; i < upsellItems.length; i++){
                  if (upsellItems[i].variant === variant) {
                    upsellItems[i].hide = "hide"
                  }
                }
              }
            }
            ...
            if(product.properties_all != undefined && product.properties_all.frequency_num != undefined && product.properties_all.frequency_type_text != undefined)
            {
              recurringDesc = "<div>Delivered every " + product.properties_all.frequency_num + " " + product.properties_all.frequency_type_text + "</div>";
            }

            if(product.selling_plan_allocation != undefined) {
              if(product.selling_plan_allocation.selling_plan.name != undefined) {
                recurringDesc = "Delivered every " + product.selling_plan_allocation.selling_plan.name;
              }
            }
      ...
      initQtySelectors: function() {
        ...
        $('body').on('qty.cart-drawer--add', this.addItem.bind(this));
      },
  
      addItem: function(evt, variant) {
  ...
  theme.headerNav = (function() {
    ...
    function init(container) {
      ...
      $wrapper = $headerContainer.find($(selectors.wrapper));
      $navContainerWithLogo = $headerContainer.find($(selectors.navContainerWithLogo));
      $logoContainer = $headerContainer.find($(selectors.logoContainer));
      $nav = $headerContainer.find($(selectors.navigation));
      $siteHeader = $headerContainer.find($(selectors.siteHeader));
    ...
    function stickyHeader(parent) {
      var parent = parent;
      
      config.lastScroll = 0;
      parent.find(selectors.siteHeader).wrap('<div class="site-header-sticky"></div>');
      stickyHeaderHeight(parent);
      
      $window.on('resize', function(){stickyHeaderHeight(parent)});
    ...
    function stickyHeaderHeight(parent) {
      var $stickyHeader = parent.find('.site-header-sticky').css('height', parent.find(selectors.siteHeader).outerHeight(true));
    }
    ...
    function stickyHeaderScroll() {
      ...
      if (scroll > threshold) {
        ...
        $(selectors.siteHeader).addClass(config.stickyClass);
        if (config.wrapperOverlayed) {
          $(selectors.wrapper).removeClass(config.overlayedClass);
        }
  
        // Add open transition class after element is set to fixed
        // so CSS animation is applied correctly
        setTimeout(function() {
          $(selectors.siteHeader).addClass(config.openTransitionClass);
        }, 100);
      } else {
        if (!config.stickyActive) {
          return;
        ...
        $(selectors.siteHeader).removeClass(config.openTransitionClass).removeClass(config.stickyClass);
        if (config.wrapperOverlayed) {
          $(selectors.wrapper).addClass(config.overlayedClass);
        }
      }
  ...
  theme.announcementBar = (function() {
    ...
    function headerBarGeo() {
      var country = '';
      var announcementContainer = $('.announcement__text');
      var announcementText = announcementContainer.data('world');
      ...
      $.get("https://ipinfo.io", function(response) {
        ...
        switch(country) {
          case 'US':
            announcementText = announcementContainer.data('us');
            break;
          case 'CA':
            announcementText = announcementContainer.data('ca');
            break;
          default:
            announcementText = announcementContainer.data('world');
        }

        announcementContainer.text(announcementText);
      }, "jsonp").fail(function() {
        announcementContainer.text(announcementText);
      });
    }
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          ...
          document.querySelector('[data-product-price]').dataset.productPrice = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat)
  ...
  theme.HeaderSection = (function() {
    ...
    function Header(container) {
      ...
      this.initDrawers();
      theme.headerNav.init($container);
    ...
    Header.prototype = $.extend({}, Header.prototype, {
      initDrawers: function() {
        ...
        theme.NavDrawer = new theme.Drawers('NavDrawerV2', 'nav');
  ...
  theme.Instagram = (function() {
    ...
    Instagram.prototype = $.extend({}, Instagram.prototype, {
      ...
      init: function() {
        ...
        this.$target.addClass('instagram-holder--active');
      }
    });
